...
-- Sessions mirror sessions/applicant_sessions/vendor_sessions exactly,
-- including the hashed-token scheme from 032 — functions/_lib/auth.js's
-- getEmployeeFromSession hashes the incoming cookie value and looks it up
-- by token_hash, so the raw session token is never written to the database.
--
-- Renumbered from this PR's original 030_employee_accounts_and_field_ops.sql
-- (030 was taken by 030_page_views_geo.sql, merged first) when this PR was
-- reconciled against main after the session-hashing security work (032).
--
