...
1. Merge the PR that added this file. Cloudflare Pages deploys `main`
   automatically.
2. `migrations/033_employee_accounts_and_field_ops.sql` and
   `migrations/034_hash_employee_session_tokens.sql` have already been
   applied to the production Neon database as part of merging this PR —
   confirm with `psql "$DATABASE_URL" -f test-schema.sql` or by checking
   that the `employee_sessions` table exists and has a `token_hash` column
   (not `token`).
