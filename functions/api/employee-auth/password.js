...
import { getEmployeeFromSession, verifyPassword, hashPassword } from '../../_lib/auth.js';
...
export async function onRequestPost({ env, request }) {
  ...
  const keep = m ? m[1] : '';
  await sql`delete from employee_sessions where employee_id = ${emp.id} and token <> ${keep}`;
